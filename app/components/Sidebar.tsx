  ...
  return (
    <aside className="w-48 bg-[#121212] flex flex-col items-start py-8 px-4 pt-8">
        <div className="flex flex-col items-start gap-1">
            <p className="text-black text-3xl font-semibold bg-pink-400 p-1">Pump</p>
            <p className="text-black text-3xl font-semibold bg-orange-400 p-1">Together</p>
        </div>
        ...
        {sidebarItems.map((item, index) => (
          <div
            ...
            className={`flex items-center gap-4 text-gray-400 text-xl cursor-pointer hover:text-yellow-400 transition duration-300 rounded-md p-4 w-48 md:bg-transparent ${
              index === 0 ? 'hover:bg-red-800' :
              index === 1 ? 'hover:bg-blue-800' :
              index === 2 ? 'hover:bg-green-800' :
              'hover:bg-purple-800'
            }`}
          >

...
class PhatBaseCommand extends BaseCommand_1.default {
    ...
    async getJsRunnerContractId(endpoint) {
    ...
    async getBrickProfileContractId({ endpoint, registry, provider, }) {
        ...
        const contract = await (0, sdk_1.getContract)({
            client: registry,
            contractId: brickProfileFactoryContractId,
            abi: brickProfileFactoryAbi,
            provider,
        });
        const { output } = await contract.q.getUserProfileAddress();
    ...
    async promptEvmAccountId({ contract, }) {
        ...
        try {
            ...
            const { output } = await contract.q.getAllEvmAccounts();
            if (output.isErr) {
    ...
    async promptRpc(message = 'Please enter your EVM RPC URL') {
    ...
    async promptBrickProfileFactory(message = 'Please enter the dashboard profile factory contract ID') {
    ...
    async promptJsRunner(message = 'Please enter the js runner contract ID') {
        const { jsRunner } = await inquirer_1.default.prompt([
            {
                name: 'jsRunner',
                type: 'input',
                message,
    ...
    async getProvider({ apiPromise, }) {
        var _a, _b, _c, _d;
        if (this.parsedFlags.privateKey || (process.env.PRIVATE_KEY && !this.parsedFlags.suri && !this.parsedFlags.accountFilePath)) {
            if (!((_b = (_a = apiPromise.consts) === null || _a === void 0 ? void 0 : _a.evmAccountMapping) === null || _b === void 0 ? void 0 : _b.eip712Name)) {
                this.action.fail('The current connected chain does not support EVM wallets.');
                this.exit(1);
            }
            const privateKey = (0, utils_1.add0xPrefix)(this.parsedFlags.privateKey || process.env.PRIVATE_KEY);
            ...
            const provider = await sdk_1.EvmAccountMappingProvider.create(apiPromise, client, account, { SS58Prefix: 30 });
            return provider;
        }
        if (this.parsedFlags.mnemonic || (process.env.MNEMONIC && !this.parsedFlags.suri && !this.parsedFlags.accountFilePath)) {
            if (!((_d = (_c = apiPromise.consts) === null || _c === void 0 ? void 0 : _c.evmAccountMapping) === null || _d === void 0 ? void 0 : _d.eip712Name)) {
                this.action.fail('The current connected chain does not support EVM wallets.');
                this.exit(1);
            }
            const addressIndex = Number(process.env.EVM_MNEMONIC_ADDRESS_INDEX || this.parsedFlags.addressIndex);
            const account = (0, accounts_1.mnemonicToAccount)(this.parsedFlags.mnemonic || process.env.MNEMONIC, { addressIndex });
            const client = (0, viem_1.createWalletClient)({
                account,
                chain: chains_1.mainnet,
                transport: (0, viem_1.http)()
        ...
        const provider = await sdk_1.KeyringPairProvider.create(apiPromise, pair);
        return provider;
    }
    async getDecodedPair({ suri, accountFilePath, accountPassword }) {
        await (0, wasm_crypto_1.waitReady)();
        const keyring = new keyring_1.Keyring({ type: 'sr25519', ss58Format: 30 });
        let pair;
        if (accountFilePath) {
    ...
    async verifyRpcEndpoint(endpoint) {
        try {
...
PhatBaseCommand.flags = {
    ...
    addressIndex: core_1.Flags.integer({

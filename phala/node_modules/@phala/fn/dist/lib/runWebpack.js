...
`;
const getBaseConfig = (buildEntries, projectDir, outputDir, development, isAsync = false) => {
    const rules = [
        {
            test: /\.ts$/,
            ...
            options: {
                context: projectDir,
                configFile: require.resolve(upath_1.default.join(__dirname, '..', '..', 'tsconfig.build.json')),
...
function removeExtension(filePath) {
    ...
    const newPath = node_path_1.default.join(parsedPath.dir, parsedPath.name);
    return newPath;
}
function modifyTargetPath(filePath) {
    return removeExtension(filePath.replace(/([^/]+)$/, '_$1'));
}
async function runWebpack({ buildEntries, projectDir, outputDir, outputFileName, customWebpack, isDev = false, clean = false, isAsync = false }) {
    ...
    const virtualModules = new webpack_virtual_modules_1.default(Object.entries(buildEntries || {}).reduce((acc, [, value]) => {
        acc[node_path_1.default.join(projectDir, modifyTargetPath(value))] = build_code_template.replace(/{filePath}/g, upath_1.default.join(projectDir, removeExtension(value)));
        return acc;
    }, {}));
    const newBuildEntries = Object.entries(buildEntries || {}).reduce((acc, [key, value]) => {
        acc[key] = upath_1.default.join(projectDir, modifyTargetPath(value));
        return acc;
    }, {});
    let config = (0, webpack_merge_1.merge)(getBaseConfig(newBuildEntries, projectDir, outputDir, isDev, isAsync), {
        output: {
            clean,

...
var require_end_of_stream = __commonJS({
  "node_modules/readable-stream/lib/internal/streams/end-of-stream.js"(exports2, module2) {
    ...
    function eos(stream, opts, callback2) {
      ...
      if (!opts)
        opts = {};
      callback2 = once4(callback2 || noop4);
...
__export(src_exports, {
  EvmAccountMappingProvider: () => EvmAccountMappingProvider,
  InkCodeSubmittableResult: () => InkCodeSubmittableResult,
  KeyringPairProvider: () => KeyringPairProvider,
  OnChainRegistry: () => OnChainRegistry,
  PhactoryAPI: () => PhactoryAPI,
  PinkBlueprintPromise: () => PinkBlueprintPromise,
  PinkBlueprintSubmittableResult: () => PinkBlueprintSubmittableResult,
  PinkCodePromise: () => PinkCodePromise,
  PinkContractPromise: () => PinkContractPromise,
  PinkLoggerContractPromise: () => PinkLoggerContractPromise,
  SignAndSendError: () => SignAndSendError,
  UIKeyringProvider: () => UIKeyringProvider,
  UnexpectedEndpointError: () => UnexpectedEndpointError,
  abis: () => abis,
  ackFirst: () => ackFirst,
  buildGetLogRequest: () => buildGetLogRequest,
  createEip712Domain: () => createEip712Domain,
  createEip712StructedDataSignCertificate: () => createEip712StructedDataSignCertificate,
  createEip712StructedDataSignQuery: () => createEip712StructedDataSignQuery,
  createEip712StructedDataSubstrateCall: () => createEip712StructedDataSubstrateCall,
  createPruntimeClient: () => createPruntimeClient,
  createSubstrateCall: () => createSubstrateCall,
  evmPublicKeyToSubstrateRawAddressU8a: () => evmPublicKeyToSubstrateRawAddressU8a,
  ...
  options: () => options,
  periodicityChecker: () => periodicityChecker,
  phalaRegistryTypes: () => phalaRegistryTypes,
  phalaTypes: () => phalaTypes,
  pruntimeRpc: () => pruntimeRpc4,
  randomHex: () => randomHex,
  recoverEvmPubkey: () => recoverEvmPubkey,
  signAndSend: () => signAndSend_default,
  signCertificate: () => signCertificate,
  signEip712Certificate: () => signEip712Certificate,
  substrateRawAddressToEvmAddress: () => substrateRawAddressToEvmAddress,
  types: () => types,
  unsafeCheckCodeHashExists: () => unsafeCheckCodeHashExists,
  unsafeGetAbiFromGitHubRepoByCodeHash: () => unsafeGetAbiFromGitHubRepoByCodeHash,
  unsafeGetAbiFromPatronByCodeHash: () => unsafeGetAbiFromPatronByCodeHash,
  unsafeGetContractCodeHash: () => unsafeGetContractCodeHash,
  unsafeGetWasmFromGithubRepoByCodeHash: () => unsafeGetWasmFromGithubRepoByCodeHash,
  unsafeGetWasmFromPatronByCodeHash: () => unsafeGetWasmFromPatronByCodeHash
});
...
var import_wallet = require("viem/wallet");

// src/options.ts
init_process();
init_buffer();
var import_types = require("@polkadot/types");
var import_substrate = __toESM(require("@polkadot/types-augment/lookup/substrate"));
var types = {
  ContractId: "H256",
...
var phalaRegistryTypes = { ...types, ...import_types.typeDefinitions, ...import_substrate.default };
var phalaTypes = new import_types.TypeRegistry();
...
function createEip712Domain(api) {
  try {
    const name = (0, import_util.hexToString)(api.consts.evmAccountMapping.eip712Name.toString());
    const version3 = (0, import_util.hexToString)(api.consts.evmAccountMapping.eip712Version.toString());
    const chainId = api.consts.evmAccountMapping.eip712ChainID.toNumber();
    const verifyingContract = api.consts.evmAccountMapping.eip712VerifyingContractAddress.toString();
    return {
      name,
      version: version3,
      chainId,
...
async function createSubstrateCall(api, substrateAddress, extrinsic) {
  const nonce = await api.query.evmAccountMapping.accountNonce(substrateAddress);
  return {
...
function createEip712StructedDataSubstrateCall(domain2, message) {
  return {
    types: {
      EIP712Domain: [
        {
...
function generatePair() {
  const generatedSeed = (0, import_util2.hexToU8a)((0, import_util2.hexAddPrefix)(randomHex(32)));
  const generatedPair = (0, import_util_crypto.sr25519PairFromSeed)(generatedSeed);
...
async function signCertificate(params) {
  ...
  const encodedCertificateBody = CertificateBody((0, import_util2.u8aToHex)(pubkey), ttl);
  let signerPubkey;
  let signature;
  let address;
  if (isUsingSigner(params)) {
    ...
    signerPubkey = (0, import_util2.u8aToHex)((0, import_util_crypto.decodeAddress)(address));
    if (!signatureType) {
      signatureType = getSignatureTypeFromAccount(account);
    }
    const signerResult = await ((_a = signer.signRaw) == null ? void 0 : _a.call(signer, {
      address,
      data: (0, import_util2.u8aToHex)(encodedCertificateBody),
      type: "bytes"
    }));
    if (signerResult) {
      signature = (0, import_util2.hexToU8a)(signerResult.signature);
    } else {
    ...
    signerPubkey = (0, import_util2.u8aToHex)(pair.publicKey);
    if (!signatureType) {
...
async function signEip712Certificate({
  client,
  account,
  compressedPubkey,
  ttl = 2147483647
  ...
  const eip712Cert = CertificateBody((0, import_util2.u8aToHex)(pubkey), ttl);
  const signature = await (0, import_wallet.signTypedData)(
    client,
    createEip712StructedDataSignCertificate(account, (0, import_util2.u8aToHex)(eip712Cert), ttl)
  );
  ...
  const certificate = {
    ...
    signature: {
      ...
      signature: (0, import_util2.hexToU8a)(signature)
    }
  };
  return { address, certificate, pubkey, secret };
}
...
var import_Abi3 = require("@polkadot/api-contract/Abi");
var import_util18 = require("@polkadot/util");

// src/contracts/PinkBlueprint.ts
init_process();
init_buffer();
var import_api2 = require("@polkadot/api");
var import_Abi2 = require("@polkadot/api-contract/Abi");
var import_util16 = require("@polkadot/api-contract/base/util");
var import_util17 = require("@polkadot/util");
var import_util_crypto5 = require("@polkadot/util-crypto");
var import_rxjs2 = require("rxjs");

// src/pruntime/coders.ts
init_process();
init_buffer();
var import_util10 = require("@polkadot/util");
...
var toU8a = (param) => {
  if (typeof param === "string") {
    param = (0, import_util9.hexAddPrefix)(param);
    return (0, import_util9.hexToU8a)(param);
  }
  return param;
};
var encrypt = (data, key, iv) => {
  data = (0, import_util9.hexStripPrefix)(data);
...
var decrypt = (enc, key, iv) => {
  enc = (0, import_util9.hexStripPrefix)(enc);
  const decipher = (0, import_browserify_cipher.createDecipheriv)(ALGO, toU8a(key), Buffer2.from(toU8a(iv)));
  const authTag = (0, import_util9.hexToU8a)((0, import_util9.hexAddPrefix)(enc.slice(-AUTH_TAG_LENGTH)));
...
// src/pruntime/coders.ts
function encodeNumber(n, defaults = import_util10.BN_ZERO) {
  if (!n) {
    return defaults;
  }
  if (typeof n === "bigint") {
...
function InkQuerySidevmMessage(address, sidevmMessage) {
  return phalaTypes.createType("InkQuery", {
    ...
    data: {
      SidevmMessage: (0, import_util10.stringToHex)(JSON.stringify(sidevmMessage))
    }
  });
}
...
function EncryptedInkCommand(address, encParams, nonce, value, gas, storageDepositLimit) {
  ...
  const commandAgreementKey = (0, import_util_crypto2.sr25519Agreement)(sk, (0, import_util10.hexToU8a)(address));
  ...
  const iv = (0, import_util10.hexAddPrefix)(randomHex(12));
  return phalaTypes.createType("CommandPayload", {
    Encrypted: {
      iv,
      pubkey: (0, import_util10.u8aToHex)(pk),
      data: (0, import_util10.hexAddPrefix)(encrypt(payload.toHex(), commandAgreementKey, (0, import_util10.hexToU8a)(iv)))
    }
  });
}
...
var import_util_crypto3 = require("@polkadot/util-crypto");
var import_proto2 = __toESM(require_proto());
function createEncryptedData(pk, data, agreementKey) {
  const iv = (0, import_util11.hexAddPrefix)(randomHex(12));
  return {
    iv,
    pubkey: (0, import_util11.u8aToHex)(pk),
    data: (0, import_util11.hexAddPrefix)(encrypt(data, agreementKey, (0, import_util11.hexToU8a)(iv)))
  };
}
async function pinkQuery(pruntimeApi, pk, queryAgreementKey, encodedQuery, { certificate, pubkey, secret }) {
  ...
  const signature = {
    ...
    signature: (0, import_util_crypto3.sr25519Sign)(encodedEncryptedData, { publicKey: pubkey, secretKey: secret })
  };
  const requestData = {
  ...
  return (0, import_util11.hexAddPrefix)(data);
}
...
init_process();
init_buffer();
var import_util12 = require("@polkadot/util");
var BN_MAX_SUPPLY = new import_util12.BN(1e12).mul(new import_util12.BN(1e9));
...
var import_Abi = require("@polkadot/api-contract/Abi");
var import_Contract = require("@polkadot/api-contract/base/Contract");
var import_util13 = require("@polkadot/api-contract/base/util");
var import_util14 = require("@polkadot/api-contract/util");
var import_util15 = require("@polkadot/util");
var import_util_crypto4 = require("@polkadot/util-crypto");
var import_rxjs = require("rxjs");
var PinkContractSubmittableResult = class extends import_Contract.ContractSubmittableResult {
  #registry;
  #isFinalized = false;
  #contract;
  #message;
  ...
  async throwsOnErrorLog(chainHeight) {
    ...
    const { records } = await logger.tail(1e3, { contract: this.#contract.address.toHex() });
    ...
    sinceSubmitted.forEach((msg) => {
      if (msg.type === "MessageOutput" && "ok" in msg.output.result) {
        ...
        if (ok.flags.length && ok.flags[0] === "Revert" && this.#message.returnType) {
          const returns = this.#contract.abi.registry.createType(this.#message.returnType.type, (0, import_util15.hexToU8a)(ok.data));
...
function createInnerProxy(callback2, path2) {
  const proxy = new Proxy(noop3, {
    get(_obj, key) {
      if (typeof key !== "string" || key === "then") {
        return void 0;
      }
      return createInnerProxy(callback2, [...path2, key]);
    },
...
var PinkContractPromise = class _PinkContractPromise {
  constructor(api, phatRegistry, abi, address, contractKey, provider) {
    this.#query = {};
    this.#tx = {};
    this._provider = void 0;
    this.qProxyInstance = void 0;
    this.execProxyInstance = void 0;
    this.#inkQuery = (isEstimating, messageOrId, options2, params) => {
      ...
      const { cert } = options2;
      const seed = (0, import_util15.hexToU8a)((0, import_util15.hexAddPrefix)(randomHex(32)));
      const pair = (0, import_util_crypto4.sr25519PairFromSeed)(seed);
      const [sk, pk] = [pair.secretKey, pair.publicKey];
      const queryAgreementKey = (0, import_util_crypto4.sr25519Agreement)(sk, (0, import_util15.hexToU8a)((0, import_util15.hexAddPrefix)(this.phatRegistry.remotePubkey)));
      const inkQueryInternal = async (origin) => {
        ...
        return {
          debugMessage,
          gasConsumed,
          gasRequired: gasRequired && !(0, import_util13.convertWeight)(gasRequired).v1Weight.isZero() ? gasRequired : gasConsumed,
      ...
      return {
        send: this._decorateMethod((origin) => (0, import_rxjs.from)(inkQueryInternal(origin)))
      };
    };
    this.#inkCommand = (messageOrId, options2, params) => {
      options2.nonce && assert((0, import_util15.isHex)(options2.nonce) && options2.nonce.length === 66, "Invalid nonce provided");
      const nonce = options2.nonce || (0, import_util15.hexAddPrefix)(randomHex(32));
      ...
      const payload = command(
        ...
        nonce,
        options2.value,
        (0, import_util13.convertWeight)(options2.gasLimit || import_util15.BN_ZERO).v2Weight,
        options2.storageDepositLimit
      );
      return this.api.tx.phalaPhatContracts.pushContractMessage(this.address, payload.toHex(), options2.deposit || import_util15.BN_ZERO).withResultTransform((result) => {
        return new PinkContractSubmittableResult(
          ...
          message,
          result,
          (0, import_util14.applyOnEvent)(result, ["ContractEmitted", "ContractExecution"], (records) => {
            return records.map(
              ({
                event: {
                  data: [, data]
    ...
    this.abi = abi instanceof import_Abi.Abi ? abi : new import_Abi.Abi(abi, api.registry.getChainProperties());
    this.api = api;
    this._decorateMethod = import_api.toPromiseMethod;
    ...
    this.abi.messages.forEach((meta) => {
      if (meta.isMutating) {
        this.#tx[meta.method] = (0, import_util13.withMeta)(
          meta,
          (options2, ...params) => {
            return this.#inkCommand(meta, options2, params);
          }
        );
        this.#query[meta.method] = (0, import_util13.withMeta)(
          meta,
  ...
  withProvider(provider) {
    return new _PinkContractPromise(this.api, this.phatRegistry, this.abi, this.address, this.contractKey, provider);
  }
  get q() {
    if (!this.qProxyInstance) {
      this.qProxyInstance = createInnerProxy(async ({ path: path2, args: [arg] }) => {
        const { args = [], origin, ...options2 } = arg || {};
        ...
        if (!options2.cert) {
          options2.cert = await this.provider.signCertificate();
          _origin = options2.cert.address;
        }
        return await this.query[key](_origin, options2, ...args);
      }, []);
    }
  ...
  get exec() {
    if (!this.execProxyInstance) {
      this.execProxyInstance = createInnerProxy(async ({ path: path2, args: [payload] }) => {
        const { args = [], waitFinalized, ..._options } = payload || {};
        const key = path2.join("::");
        if (!this.provider) {
          throw new Error("The provider is not set");
        }
        ...
        const submittableResult = await this._send(key, options2, ...args);
        if (waitFinalized) {
    ...
    return new Proxy(
      {},
      {
        get: (_target, prop, _receiver) => {
          ...
          return (0, import_util13.withMeta)(meta[0], (options2, ...arags) => {
            return this._send(prop, options2, ...arags);
          });
        }
      }
    );
  ...
  #inkQuery;
  #inkCommand;
  async _send(messageOrId, options2, ...args) {
    ...
    const storageDepositFee = storageDeposit.isCharge ? storageDeposit.asCharge.toBn() : import_util15.BN_ZERO;
    ...
    if ("provider" in rest) {
      options2.nonce && assert((0, import_util15.isHex)(options2.nonce) && options2.nonce.length === 66, "Invalid nonce provided");
      const nonce = options2.nonce || (0, import_util15.hexAddPrefix)(randomHex(32));
      return await rest.provider.send(tx(txOptions, ...args), (result) => {
        return new PinkContractSubmittableResult(
          this.phatRegistry,
          this,
          nonce,
...
// src/contracts/PinkBlueprint.ts
var PinkBlueprintSubmittableResult = class extends import_api2.SubmittableResult {
  #contractId;
  #isFinalized;
  #contract;
...
var PinkBlueprintPromise = class {
  constructor(api, phatRegistry, abi, codeHash) {
    this.#query = {};
    this.#tx = {};
    this.#deploy = (constructorOrId, { gasLimit = import_util17.BN_ZERO, storageDepositLimit = null, value = import_util17.BN_ZERO, deposit = import_util17.BN_ZERO, salt }, params) => {
      if (!salt) {
        salt = randomHex(4);
      }
    ...
    this.#estimateGas = (constructorOrId, options2, params) => {
      const seed = (0, import_util17.hexToU8a)((0, import_util17.hexAddPrefix)(randomHex(32)));
      const pair = (0, import_util_crypto5.sr25519PairFromSeed)(seed);
      const [sk, pk] = [pair.secretKey, pair.publicKey];
      const { cert } = options2;
      const queryAgreementKey = (0, import_util_crypto5.sr25519Agreement)(sk, (0, import_util17.hexToU8a)((0, import_util17.hexAddPrefix)(this.phatRegistry.remotePubkey)));
      const inkQueryInternal = async (origin) => {
        if (typeof origin === "string") {
          assert(origin === cert.address, "origin must be the same as the certificate address");
        } else if (origin.hasOwnProperty("verify") && origin.hasOwnProperty("adddress")) {
    ...
    this.abi = abi instanceof import_Abi2.Abi ? abi : new import_Abi2.Abi(abi, api.registry.getChainProperties());
    this.api = api;
    this._decorateMethod = import_api2.toPromiseMethod;
    this.phatRegistry = phatRegistry;
    this.codeHash = phalaTypes.createType("Hash", codeHash);
    this.abi.constructors.forEach((meta) => {
      if ((0, import_util17.isUndefined)(this.#tx[meta.method])) {
        this.#tx[meta.method] = (0, import_util16.createBluePrintTx)(
          meta,
          (o, p) => this.#deploy(meta, o, p)
        );
        this.#query[meta.method] = (0, import_util16.withMeta)(
          meta,
    ...
    return new Proxy(
      {},
      {
        get: (_target, prop, _receiver) => {
          ...
          return (0, import_util16.withMeta)(meta[0], (options2, ...arags) => {
            return this._send(prop, options2, ...arags);
          });
        }
      }
  ...
  #deploy;
  #estimateGas;
  async _send(constructorOrId, options2, ...args) {
    const { cert: userCert, ...rest } = options2;
    const txOptions = {
      gasLimit: options2.gasLimit,
      value: options2.value,
    ...
    const gasLimit = gasRequired.refTime.toBn();
    const storageDepositFee = storageDeposit.isCharge ? storageDeposit.asCharge.toBn() : import_util17.BN_ZERO;
    const minRequired = gasLimit.mul(gasPrice).add(storageDepositFee);
    if (clusterBalance.free.lt(minRequired)) {
      const deposit = minRequired.sub(clusterBalance.free);
      if (onchainBalance.data.free.lt(deposit)) {
...
// src/contracts/PinkCode.ts
var InkCodeSubmittableResult = class extends import_api3.SubmittableResult {
  #isFinalized;
...
var PinkCodePromise = class {
  constructor(api, phatRegistry, abi, wasm) {
    ...
    this.#instantiate = (_constructorOrId, _params) => {
      return this.api.tx.phalaPhatContracts.clusterUploadResource(this.phatRegistry.clusterId, "InkCode", (0, import_util18.u8aToHex)(this.code)).withResultTransform((result) => {
    ...
    this.abi = abi instanceof import_Abi3.Abi ? abi : new import_Abi3.Abi(abi, api.registry.getChainProperties());
    this.api = api;
    this._decorateMethod = import_api3.toPromiseMethod;
    this.phatRegistry = phatRegistry;
    if ((0, import_util18.isWasm)(this.abi.info.source.wasm)) {
      this.code = this.abi.info.source.wasm;
    } else if ((0, import_util18.isU8a)(wasm)) {
      this.code = wasm;
    } else if (typeof wasm === "string" && wasm.substring(0, 2) === "0x") {
      this.code = (0, import_util18.hexToU8a)(wasm);
    } else {
      throw new Error("`wasm` should hex encoded string or Uint8Array.");
    }
    if (!(0, import_util18.isWasm)(this.code)) {
  ...
  async send({ provider }) {
    return await provider.send(
      this.api.tx.phalaPhatContracts.clusterUploadResource(this.phatRegistry.clusterId, "InkCode", (0, import_util18.u8aToHex)(this.code)),
...
var import_util19 = require("@polkadot/util");
var import_util_crypto6 = require("@polkadot/util-crypto");
...
function sidevmQueryWithReader({ phactory, remotePubkey, address, cert }) {
  return async function unsafeRunSidevmQuery(sidevmMessage) {
    ...
    const queryAgreementKey = (0, import_util_crypto6.sr25519Agreement)(sk, (0, import_util19.hexToU8a)((0, import_util19.hexAddPrefix)(remotePubkey)));
    ...
    const parsed = payload.substring(0, 2) === "0x" ? JSON.parse((0, import_util19.hexToString)(payload)) : JSON.parse(payload);
    if (parsed.error) {
...
function postProcessLogRecord(messages, abiLike) {
  let abi;
  if (abiLike) {
    abi = abiLike instanceof import_api_contract.Abi ? abiLike : new import_api_contract.Abi(abiLike);
  }
  return messages.map((message) => {
    var _a;
    if (message.type === "MessageOutput") {
      const execResult = phalaTypes.createType("ContractExecResult", (0, import_util19.hexToU8a)(message.output));
      ...
      try {
        const decoded = abi.decodeEvent((0, import_util19.hexToU8a)(message.payload));
...
function getTopicHash(topic) {
  ...
  if (encoded.length > 32) {
    return (0, import_util19.u8aToHex)((0, import_util_crypto6.blake2AsU8a)(encoded));
  } else {
    return (0, import_util19.u8aToHex)(encoded) + "00".repeat(32 - encoded.length);
  }
}
var PinkLoggerContractPromise = class _PinkLoggerContractPromise {
  ...
  static async create(_api, registry, systemContract, pair) {
    ...
    let _pair2 = pair;
    if (!_pair2) {
      const keyring = new import_api4.Keyring({ type: "sr25519" });
      _pair2 = keyring.addFromUri("//Alice");
    }
    const cert = await signCertificate({ pair: _pair2 });
    const { output } = await systemContract.query["system::getDriver"](_pair2.address, { cert }, "PinkLogger");
    ...
    return new _PinkLoggerContractPromise(registry.phactory, registry.remotePubkey, _pair2, contractId, systemContractId);
  }
...
init_process();
init_buffer();
var import_api6 = require("@polkadot/api");
var import_util_crypto8 = require("@polkadot/util-crypto");
...
function createPruntimeClient(baseURL) {
  const pruntimeApi = import_proto3.pruntime_rpc.PhactoryAPI.create(async (method, requestData, callback2) => {
    try {
      const resp = await (0, import_cross_fetch.default)(`${baseURL}/prpc/PhactoryAPI.${method.name}`, {
        method: "POST",
        headers: {
...
// src/providers/KeyringPairProvider.ts
init_process();
init_buffer();
var import_api5 = require("@polkadot/api");
var import_util_crypto7 = require("@polkadot/util-crypto");
var _apiPromise, _pair, _cachedCert, _certExpiredAt;
var _KeyringPairProvider = class _KeyringPairProvider {
  constructor(api, pair) {
    __privateAdd(this, _apiPromise, void 0);
    __privateAdd(this, _pair, void 0);
    __privateAdd(this, _cachedCert, void 0);
    __privateAdd(this, _certExpiredAt, void 0);
    __privateSet(this, _apiPromise, api);
    __privateSet(this, _pair, pair);
  }
  static async create(api, pair) {
    return new _KeyringPairProvider(api, pair);
  }
  static async createFromSURI(api, suri, options2) {
    await (0, import_util_crypto7.cryptoWaitReady)();
    const { ss58Format, meta, type } = options2 || {};
    const keyring = new import_api5.Keyring({ type, ss58Format });
    const pair = keyring.addFromUri(suri, meta, type);
    return new _KeyringPairProvider(api, pair);
  }
  get address() {
    return __privateGet(this, _pair).address;
  }
  get name() {
    return _KeyringPairProvider.identity;
  }
  /**
   * Send an extrinsic to the network.
   */
  async send(extrinsic, transform) {
    const result = await signAndSend_default(extrinsic, __privateGet(this, _pair));
    if (transform) {
   ...
   */
  async signCertificate(ttl = 2147483647) {
    const now = Date.now();
    const isExpired = __privateGet(this, _certExpiredAt) && __privateGet(this, _certExpiredAt) < now;
    if (__privateGet(this, _cachedCert) && !isExpired) {
      return __privateGet(this, _cachedCert);
    }
    __privateSet(this, _cachedCert, await signCertificate({ pair: __privateGet(this, _pair), ttl }));
    __privateSet(this, _certExpiredAt, now + ttl * 1e3);
    return __privateGet(this, _cachedCert);
  }
  get isCertificateExpired() {
    if (!__privateGet(this, _cachedCert)) {
      return true;
    }
    const now = Date.now();
    return !!(__privateGet(this, _certExpiredAt) && __privateGet(this, _certExpiredAt) < now);
  }
  get hasCertificate() {
    const now = Date.now();
    return !!(__privateGet(this, _cachedCert) && !(__privateGet(this, _certExpiredAt) && __privateGet(this, _certExpiredAt) < now));
  }
  revokeCertificate() {
    __privateSet(this, _cachedCert, void 0);
    __privateSet(this, _certExpiredAt, void 0);
  }
  async adjustStake(contractId, amount) {
    await this.send(__privateGet(this, _apiPromise).tx.phalaPhatTokenomic.adjustStake(contractId, amount));
  }
};
_apiPromise = new WeakMap();
_pair = new WeakMap();
_cachedCert = new WeakMap();
_certExpiredAt = new WeakMap();
_KeyringPairProvider.identity = "keyring";
var KeyringPairProvider = _KeyringPairProvider;
...
var OnChainRegistry = class _OnChainRegistry {
  ...
  static async create(api, options2) {
    ...
    await Promise.all([(0, import_util_crypto8.cryptoWaitReady)(), api.isReady]);
    if (options2.autoConnect) {
      ...
      } else if (options2.clusterId && !options2.strategy) {
        await instance.connect({
          clusterId: options2.clusterId,
          strategy: "ack-first"
        });
  ...
  get alice() {
    if (!this.#alice) {
      const keyring = new import_api6.Keyring({ type: "sr25519" });
  ...
  get isEvmAccountMappingSupported() {
    var _a, _b;
    return !!((_b = (_a = this.api.consts) == null ? void 0 : _a.evmAccountMapping) == null ? void 0 : _b.eip712Name);
  }
};
...
async function unsafeGetAbiFromPatronByCodeHash(codeHash) {
  ...
  const resp = await (0, import_cross_fetch2.default)(`https://api.patron.works/buildSessions/metadata/${codeHashWithoutPrefix}`);
  if (resp.status !== 200) {
    let payload;
    try {
...
async function unsafeGetAbiFromGitHubRepoByCodeHash(codeHash) {
  ...
  const resp = await (0, import_cross_fetch2.default)(`${OFFICIAL_ARTIFACTS_URL}/artifacts/${codeHashWithPrefix}/metadata.json`);
  if (resp.status !== 200) {
...
async function unsafeGetWasmFromPatronByCodeHash(codeHash) {
  const codeHashWithoutPrefix = codeHash.indexOf("0x") === 0 ? codeHash.replace("0x", "") : codeHash;
  const resp = await (0, import_cross_fetch2.default)(`https://api.patron.works/buildSessions/wasm/${codeHashWithoutPrefix}`);
  if (resp.status !== 200) {
...
async function unsafeGetWasmFromGithubRepoByCodeHash(codeHash) {
  const codeHashWithPrefix = codeHash.indexOf("0x") !== 0 ? `0x${codeHash}` : codeHash;
  const resp = await (0, import_cross_fetch2.default)(`${OFFICIAL_ARTIFACTS_URL}/artifacts/${codeHashWithPrefix}/out.wasm`);
  if (resp.status !== 200) {
...
// src/utils/addressConverter.ts
init_process();
init_buffer();
var import_util20 = require("@polkadot/util");
var import_util_crypto9 = require("@polkadot/util-crypto");
var import_viem = require("viem");
var import_wallet2 = require("viem/wallet");
async function recoverEvmPubkey(client, account, msg = "Allows to access the pubkey address.") {
  const sign = await (0, import_wallet2.signMessage)(client, { account, message: msg });
  const hash = (0, import_viem.hashMessage)(msg);
  const uncompressed = await (0, import_viem.recoverPublicKey)({ hash, signature: sign });
  const compressed = (0, import_util20.u8aToHex)((0, import_util_crypto9.secp256k1Compress)((0, import_util20.hexToU8a)(uncompressed)));
  return {
    uncompressed,
    compressed,
...
function evmPublicKeyToSubstrateRawAddressU8a(hex, converter = "EvmTransparentConverter") {
  let pubkey = (0, import_util20.hexToU8a)(hex);
  ...
  if (converter === "EvmTransparentConverter") {
    const h32 = (0, import_util_crypto9.keccak256AsU8a)(isCompressedPubkey ? (0, import_util_crypto9.secp256k1Expand)(pubkey) : pubkey.subarray(1));
    ...
    if (!isCompressedPubkey) {
      pubkey = (0, import_util_crypto9.secp256k1Compress)(pubkey);
    }
    return (0, import_util_crypto9.blake2AsU8a)(pubkey);
  } else {
    throw new Error(`Unknown converter: ${converter}`);
  }
}
function substrateRawAddressToEvmAddress(address) {
  const substratePubkey = (0, import_util_crypto9.decodeAddress)(address);
  ...
  return (0, import_util20.u8aToHex)(h20);
}
...
// src/providers/EvmAccountMappingProvider.ts
init_process();
init_buffer();
var import_util_crypto10 = require("@polkadot/util-crypto");
var _apiPromise2, _client, _account, _SS58Prefix, _domain, _address, _recoveredPubkey, _cachedCert2, _certExpiredAt2;
var _EvmAccountMappingProvider = class _EvmAccountMappingProvider {
  constructor(api, client, account, { SS58Prefix = void 0 } = {}) {
    //
    // Resources
    //
    __privateAdd(this, _apiPromise2, void 0);
    __privateAdd(this, _client, void 0);
    __privateAdd(this, _account, void 0);
    //
    // Options
    //
    __privateAdd(this, _SS58Prefix, void 0);
    //
    // State
    //
    ...
    __privateAdd(this, _recoveredPubkey, void 0);
    __privateAdd(this, _cachedCert2, void 0);
    __privateAdd(this, _certExpiredAt2, void 0);
    var _a;
    __privateSet(this, _apiPromise2, api);
    __privateSet(this, _client, client);
    __privateSet(this, _account, account);
    __privateSet(this, _domain, createEip712Domain(api));
    __privateSet(this, _SS58Prefix, SS58Prefix || ((_a = api.consts.system) == null ? void 0 : _a.ss58Prefix).toNumber() || 30);
  }
  get name() {
    return _EvmAccountMappingProvider.identity;
  }
  async ready(msg) {
    const version3 = __privateGet(this, _apiPromise2).consts.evmAccountMapping.eip712Version.toString();
    if (version3 !== "0x31" && version3 !== "0x32") {
      throw new Error(
        `Unsupported evm_account_mapping pallet version: consts.evmAccountMapping.eip712Version = ${version3}`
      );
    }
    __privateSet(this, _recoveredPubkey, await recoverEvmPubkey(__privateGet(this, _client), __privateGet(this, _account), msg));
    const converter = version3 === "0x32" ? "EvmTransparentConverter" : "SubstrateAddressConverter";
    __privateSet(this, _address, (0, import_util_crypto10.encodeAddress)(
      evmPublicKeyToSubstrateRawAddressU8a(__privateGet(this, _recoveredPubkey).compressed, converter),
      __privateGet(this, _SS58Prefix)
    ));
  }
  static async create(api, client, account, options2) {
    const signer = new _EvmAccountMappingProvider(api, client, account, options2);
    await signer.ready(options2 == null ? void 0 : options2.msg);
    return signer;
  }
  /**
  ...
  get compressedPubkey() {
    if (!__privateGet(this, _recoveredPubkey)) {
      throw new Error("WalletClientSigner is not ready.");
    }
    return __privateGet(this, _recoveredPubkey).compressed;
  }
  get evmCaller() {
    if (!__privateGet(this, _recoveredPubkey)) {
      throw new Error("WalletClientSigner is not ready.");
    ...
    return {
      compressedPubkey: __privateGet(this, _recoveredPubkey).compressed,
   ...
   */
  async send(extrinsic, transform) {
    const substrateCall = await createSubstrateCall(__privateGet(this, _apiPromise2), this.address, extrinsic);
    const typedData = createEip712StructedDataSubstrateCall(__privateGet(this, _domain), substrateCall);
    const signature = await __privateGet(this, _client).signTypedData({ ...typedData, account: __privateGet(this, _account) });
    return await new Promise(async (resolve, reject) => {
      try {
        const _extrinsic = __privateGet(this, _apiPromise2).tx.evmAccountMapping.metaCall(
          this.address,
  ...
  async signCertificate(ttl = 2147483647) {
    ...
    const now = Date.now();
    const isExpired = __privateGet(this, _certExpiredAt2) && __privateGet(this, _certExpiredAt2) < now;
    if (__privateGet(this, _cachedCert2) && !isExpired) {
      return __privateGet(this, _cachedCert2);
    }
    __privateSet(this, _cachedCert2, await signEip712Certificate({
      client: __privateGet(this, _client),
      account: __privateGet(this, _account),
      compressedPubkey: __privateGet(this, _recoveredPubkey).compressed,
      ttl
    }));
    __privateSet(this, _certExpiredAt2, now + ttl * 1e3);
    return __privateGet(this, _cachedCert2);
  }
  get isCertificateExpired() {
    if (!__privateGet(this, _cachedCert2)) {
      return true;
    }
    const now = Date.now();
    return !!(__privateGet(this, _certExpiredAt2) && __privateGet(this, _certExpiredAt2) < now);
  }
  get hasCertificate() {
    const now = Date.now();
    return !!(__privateGet(this, _cachedCert2) && !(__privateGet(this, _certExpiredAt2) && __privateGet(this, _certExpiredAt2) < now));
  }
  revokeCertificate() {
    __privateSet(this, _cachedCert2, void 0);
    __privateSet(this, _certExpiredAt2, void 0);
  }
  async adjustStake(contractId, amount) {
    await this.send(__privateGet(this, _apiPromise2).tx.phalaPhatTokenomic.adjustStake(contractId, amount));
  }
};
_apiPromise2 = new WeakMap();
_client = new WeakMap();
_account = new WeakMap();
_SS58Prefix = new WeakMap();
_domain = new WeakMap();
_address = new WeakMap();
_recoveredPubkey = new WeakMap();
_cachedCert2 = new WeakMap();
_certExpiredAt2 = new WeakMap();
_EvmAccountMappingProvider.identity = "evmAccountMapping";
var EvmAccountMappingProvider = _EvmAccountMappingProvider;

// src/providers/UIKeyringProvider.ts
init_process();
init_buffer();
var import_ui_keyring = require("@polkadot/ui-keyring");
var import_util_crypto11 = require("@polkadot/util-crypto");
...
var checkInstalledWalletExtensions = false;
var installedWalletExtensions = [];
async function getInstalledWalletExtensions() {
  ...
  installedWalletExtensions = (0, import_ramda.map)(
    ([k, v]) => [k, v.version],
    (0, import_ramda.toPairs)((0, import_ramda.propOr)({}, "injectedWeb3", window))
  );
...
async function getKeyring() {
  ...
  const keyring = new import_ui_keyring.Keyring();
  try {
...
var _apiPromise3, _signer, _account2, _cachedCert3, _certExpiredAt3;
var _UIKeyringProvider = class _UIKeyringProvider {
  constructor(api, signer, account) {
    //
    // Resources
    //
    __privateAdd(this, _apiPromise3, void 0);
    __privateAdd(this, _signer, void 0);
    __privateAdd(this, _account2, void 0);
    __privateAdd(this, _cachedCert3, void 0);
    __privateAdd(this, _certExpiredAt3, void 0);
    __privateSet(this, _apiPromise3, api);
    __privateSet(this, _signer, signer);
    __privateSet(this, _account2, account);
  }
  static async create(api, appName, providerName, account) {
    const provider = (0, import_ramda.path)(["injectedWeb3", providerName], window);
    ...
    return new _UIKeyringProvider(api, gateway.signer, account);
  }
  get address() {
    return __privateGet(this, _account2).address;
  }
  get name() {
    return _UIKeyringProvider.identity;
  }
  get account() {
    return __privateGet(this, _account2);
  }
  /**
   * Send an extrinsic to the network.
   */
  async send(extrinsic, transform) {
    const result = await signAndSend_default(extrinsic, __privateGet(this, _account2).address, __privateGet(this, _signer));
    if (transform) {
      return transform(result);
    }
    return result;
  ...
  async signCertificate(ttl = 2147483647) {
    const now = Date.now();
    const isExpired = __privateGet(this, _certExpiredAt3) && __privateGet(this, _certExpiredAt3) < now;
    if (__privateGet(this, _cachedCert3) && !isExpired) {
      return __privateGet(this, _cachedCert3);
    }
    __privateSet(this, _cachedCert3, await signCertificate({ signer: __privateGet(this, _signer), account: __privateGet(this, _account2), ttl }));
    __privateSet(this, _certExpiredAt3, now + ttl * 1e3);
    return __privateGet(this, _cachedCert3);
  }
  get isCertificateExpired() {
    if (!__privateGet(this, _cachedCert3)) {
      return true;
    }
    const now = Date.now();
    return !!(__privateGet(this, _certExpiredAt3) && __privateGet(this, _certExpiredAt3) < now);
  }
  get hasCertificate() {
    const now = Date.now();
    return !!(__privateGet(this, _cachedCert3) && !(__privateGet(this, _certExpiredAt3) && __privateGet(this, _certExpiredAt3) < now));
  }
  revokeCertificate() {
    __privateSet(this, _cachedCert3, void 0);
    __privateSet(this, _certExpiredAt3, void 0);
  }
  async adjustStake(contractId, amount) {
    await this.send(__privateGet(this, _apiPromise3).tx.phalaPhatTokenomic.adjustStake(contractId, amount));
  }
  //
  // Extra methods specified for Web UI.
  //
  static async getSupportedWallets() {
    const availables = (0, import_ramda.fromPairs)(await getInstalledWalletExtensions());
  ...
  static async getAllAccountsFromProvider(appName, providerName) {
    const provider = (0, import_ramda.path)(["injectedWeb3", providerName], window);
    if (provider && provider.enable) {
      try {
        ...
        return (0, import_ramda.sort)(
          (a, b) => (a.name || "").localeCompare(b.name || ""),
          accounts.map(
            (acc) => ({
              ...acc,
              address: keyring.encodeAddress((0, import_util_crypto11.decodeAddress)(acc.address), 30)
            })
          )
        );
      } catch (_err) {
...
_apiPromise3 = new WeakMap();
_signer = new WeakMap();
_account2 = new WeakMap();
_cachedCert3 = new WeakMap();
_certExpiredAt3 = new WeakMap();
_UIKeyringProvider.identity = "uiKeyring";
var UIKeyringProvider = _UIKeyringProvider;
...
function periodicityChecker(url) {
  return async function(_apiPromise4, clusterId) {
    const resp = await (0, import_cross_fetch3.default)(url || PRUNTIME_NODE_LIST);
...
function fixture(endpoints) {
  return async function(_apiPromise4, _clusterId) {
    if (endpoints.length === 0) {
      throw new Error("No worker available.");
    }
    const picked = endpoints[Math.floor(Math.random() * endpoints.length)];
...
init_process();
init_buffer();
var import_api7 = require("@polkadot/api");
var import_util_crypto12 = require("@polkadot/util-crypto");

// src/utils/fetchMetadata.ts
init_process();
init_buffer();
var import_cross_fetch4 = __toESM(require("cross-fetch"));
async function requestRpc(http, body) {
  return (0, import_cross_fetch4.default)(http, {
    method: "POST",
    body,
    headers: { "content-type": "application/json" }
  }).then((resp) => resp.json());
}
async function fetchMetadata(rpc) {
...
async function getClient(opts) {
  ...
  const provider = typeof transport === "string" ? new import_api7.WsProvider(transport) : transport;
  ...
  const api = await import_api7.ApiPromise.create(options({ provider, noInitWarn: true, metadata }));
...
async function getContract(options2) {
  ...
  return new PinkContractPromise(client.api, client, abi, contractId, contractKey, options2.provider);
}
async function getLogger(options2) {
  if ("transport" in options2) {
    ...
    await (0, import_util_crypto12.cryptoWaitReady)();
    const keyring = new import_api7.Keyring({ type: "sr25519" });

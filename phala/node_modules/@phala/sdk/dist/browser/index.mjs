...
function createEip712Domain(api) {
  try {
    ...
    const version3 = hexToString(api.consts.evmAccountMapping.eip712Version.toString());
...
import { hexToU8a as hexToU8a7, isU8a, isWasm, u8aToHex as u8aToHex4 } from "@polkadot/util";
...
import { BN_ZERO as BN_ZERO3, hexAddPrefix as hexAddPrefix6, hexToU8a as hexToU8a6, isUndefined as isUndefined2 } from "@polkadot/util";
...
init_process();
init_buffer();
import { BN, BN_ZERO, hexAddPrefix as hexAddPrefix3, hexToU8a as hexToU8a3, stringToHex, u8aToHex as u8aToHex2 } from "@polkadot/util";
...
import { hexAddPrefix as hexAddPrefix2, hexStripPrefix, hexToU8a as hexToU8a2 } from "@polkadot/util";
var ALGO = "aes-256-gcm";
var AUTH_TAG_LENGTH = 32;
var toU8a = (param) => {
  if (typeof param === "string") {
    param = hexAddPrefix2(param);
    return hexToU8a2(param);
  }
  return param;
};
...
var decrypt = (enc, key, iv) => {
  ...
  const authTag = hexToU8a2(hexAddPrefix2(enc.slice(-AUTH_TAG_LENGTH)));
...
function EncryptedInkCommand(address, encParams, nonce, value, gas, storageDepositLimit) {
  ...
  const commandAgreementKey = sr25519Agreement(sk, hexToU8a3(address));
  ...
  return phalaTypes.createType("CommandPayload", {
    Encrypted: {
      iv,
      pubkey: u8aToHex2(pk),
      data: hexAddPrefix3(encrypt(payload.toHex(), commandAgreementKey, hexToU8a3(iv)))
    }
  });
}
...
init_process();
init_buffer();
import { hexAddPrefix as hexAddPrefix4, hexToU8a as hexToU8a4, u8aToHex as u8aToHex3 } from "@polkadot/util";
...
import { BN_ZERO as BN_ZERO2, hexAddPrefix as hexAddPrefix5, hexToU8a as hexToU8a5, isHex } from "@polkadot/util";
    ...
    sinceSubmitted.forEach((msg) => {
      if (msg.type === "MessageOutput" && "ok" in msg.output.result) {
        ...
        if (ok.flags.length && ok.flags[0] === "Revert" && this.#message.returnType) {
          const returns = this.#contract.abi.registry.createType(this.#message.returnType.type, hexToU8a5(ok.data));
...
var PinkContractPromise = class _PinkContractPromise {
  constructor(api, phatRegistry, abi, address, contractKey, provider) {
    ...
    this.#inkQuery = (isEstimating, messageOrId, options2, params) => {
      ...
      const queryAgreementKey = sr25519Agreement2(sk, hexToU8a5(hexAddPrefix5(this.phatRegistry.remotePubkey)));
...
var PinkBlueprintPromise = class {
  constructor(api, phatRegistry, abi, codeHash) {
    ...
    this.#estimateGas = (constructorOrId, options2, params) => {
      ...
      const [sk, pk] = [pair.secretKey, pair.publicKey];
      const { cert } = options2;
      const queryAgreementKey = sr25519Agreement3(sk, hexToU8a6(hexAddPrefix6(this.phatRegistry.remotePubkey)));
      const inkQueryInternal = async (origin) => {
        if (typeof origin === "string") {
          assert(origin === cert.address, "origin must be the same as the certificate address");
        } else if (origin.hasOwnProperty("verify") && origin.hasOwnProperty("adddress")) {
...
var PinkCodePromise = class {
  constructor(api, phatRegistry, abi, wasm) {
    ...
    this.#instantiate = (_constructorOrId, _params) => {
      return this.api.tx.phalaPhatContracts.clusterUploadResource(this.phatRegistry.clusterId, "InkCode", u8aToHex4(this.code)).withResultTransform((result) => {
  ...
  async send({ provider }) {
    return await provider.send(
      this.api.tx.phalaPhatContracts.clusterUploadResource(this.phatRegistry.clusterId, "InkCode", u8aToHex4(this.code)),
...
import { hexAddPrefix as hexAddPrefix7, hexToString as hexToString2, hexToU8a as hexToU8a8, u8aToHex as u8aToHex5 } from "@polkadot/util";
import { blake2AsU8a, sr25519Agreement as sr25519Agreement4 } from "@polkadot/util-crypto";
...
function sidevmQueryWithReader({ phactory, remotePubkey, address, cert }) {
  return async function unsafeRunSidevmQuery(sidevmMessage) {
    ...
    const queryAgreementKey = sr25519Agreement4(sk, hexToU8a8(hexAddPrefix7(remotePubkey)));
    ...
    const parsed = payload.substring(0, 2) === "0x" ? JSON.parse(hexToString2(payload)) : JSON.parse(payload);
    if (parsed.error) {
...
function postProcessLogRecord(messages, abiLike) {
  ...
  return messages.map((message) => {
    ...
    if (message.type === "MessageOutput") {
      const execResult = phalaTypes.createType("ContractExecResult", hexToU8a8(message.output));
      ...
      try {
        const decoded = abi.decodeEvent(hexToU8a8(message.payload));
...
function getTopicHash(topic) {
  ...
  if (encoded.length > 32) {
    return u8aToHex5(blake2AsU8a(encoded));
  } else {
    return u8aToHex5(encoded) + "00".repeat(32 - encoded.length);
  }
}
...
var _KeyringPairProvider = class _KeyringPairProvider {
  ...
  static async createFromSURI(api, suri, options2) {
    ...
    const keyring = new Keyring2({ type, ss58Format });
...
var OnChainRegistry = class _OnChainRegistry {
  ...
  static async create(api, options2) {
    ...
    await Promise.all([cryptoWaitReady3(), api.isReady]);
...
init_process();
init_buffer();
import { hexToU8a as hexToU8a9, u8aToHex as u8aToHex6 } from "@polkadot/util";
import { blake2AsU8a as blake2AsU8a2, decodeAddress as decodeAddress2, keccak256AsU8a, secp256k1Compress, secp256k1Expand } from "@polkadot/util-crypto";
...
async function recoverEvmPubkey(client, account, msg = "Allows to access the pubkey address.") {
  ...
  const compressed = u8aToHex6(secp256k1Compress(hexToU8a9(uncompressed)));
  return {
    uncompressed,
    compressed,
...
var _EvmAccountMappingProvider = class _EvmAccountMappingProvider {
  ...
  async ready(msg) {
    ...
    __privateSet(this, _address, encodeAddress(
...
init_process();
init_buffer();
import { Keyring as Keyring4 } from "@polkadot/ui-keyring";
...
async function getInstalledWalletExtensions() {
  ...
  installedWalletExtensions = map(
    ([k, v]) => [k, v.version],
    toPairs(propOr({}, "injectedWeb3", window))
  );
...
var _UIKeyringProvider = class _UIKeyringProvider {
  ...
  static async create(api, appName, providerName, account) {
    const provider = path(["injectedWeb3", providerName], window);
      ...
      try {
        ...
        return sort(
          ...
          accounts.map(
            (acc) => ({
              ...acc,
              address: keyring.encodeAddress(decodeAddress3(acc.address), 30)
            })
          )
        );
      } catch (_err) {
...
init_process();
init_buffer();
import { ApiPromise, Keyring as Keyring5, WsProvider } from "@polkadot/api";
import { cryptoWaitReady as cryptoWaitReady4 } from "@polkadot/util-crypto";
...
async function getLogger(options2) {
  if ("transport" in options2) {
    ...
    await cryptoWaitReady4();
    const keyring = new Keyring5({ type: "sr25519" });

...
import { Signer as Signer$1, SubmittableExtrinsic, DecorateMethod, ApiTypes, ApiOptions } from '@polkadot/api/types';
import { KeyringPair, KeyringPair$Meta } from '@polkadot/keyring/types';
import * as _polkadot_types_types from '@polkadot/types/types';
import { Signer, ISubmittableResult, Codec, IEnum, IKeyringPair, Registry, ITuple, RegistryTypes } from '@polkadot/types/types';
import { KeypairType, Prefix } from '@polkadot/util-crypto/types';
import { Client, Account, Address, WalletClient, TestClient, Hex } from 'viem';
import { ApiBase } from '@polkadot/api/base';
import { Abi } from '@polkadot/api-contract/Abi';
import { MessageMeta, ContractCallResult } from '@polkadot/api-contract/base/types';
...
import { Enum, Vec, u8, Map, Result, Struct, u32, u128, u64, u16, Option, Bool, U8aFixed, Text, TypeRegistry } from '@polkadot/types';
import { BN } from '@polkadot/util';
import { ContractCallOutcome, ContractOptions, AbiMessage, DecodedEvent, BlueprintOptions } from '@polkadot/api-contract/types';
import { VecFixed } from '@polkadot/types/codec';
import { SubmittableExtrinsic as SubmittableExtrinsic$1 } from '@polkadot/api/submittable/types';
import { ContractSubmittableResult } from '@polkadot/api-contract/base/Contract';
import { signTypedData } from 'viem/wallet';
import { ApiTypes as ApiTypes$1 } from '@polkadot/api-base/types/base';
import { SubmittableExtrinsic as SubmittableExtrinsic$2, AddressOrPair } from '@polkadot/api-base/types/submittable';
...
declare function generatePair(): [Uint8Array, Uint8Array];
declare function signCertificate(params: CertificateParams): Promise<CertificateData>;
declare function signEip712Certificate({ client, account, compressedPubkey, ttl, }: {
    client: Client;
    account: Account;
...
interface Provider {
     ...
     */
    send<TSubmittableResult extends SubmittableResult = SubmittableResult>(extrinsic: SubmittableExtrinsic<'promise', TSubmittableResult | ISubmittableResult>, transform?: (input: ISubmittableResult) => TSubmittableResult): Promise<TSubmittableResult>;
    /**
...
type PinkContractCallOutcome<ResultType> = {
    output: ResultType;
} & Omit<ContractCallOutcome, 'output'>;
interface ILooseResult<O, E extends Codec = Codec> extends IEnum {
    readonly asErr: E;
    readonly asOk: O;
    readonly isErr: boolean;
    readonly isOk: boolean;
}
interface PinkContractQuery<TParams extends Array<any> = any[], DefaultResultType = Codec, DefaultErrType extends Codec = Codec> extends MessageMeta {
    <ResultType = DefaultResultType, ErrType extends Codec = DefaultErrType>(origin: string | AccountId | Uint8Array, options: PinkContractQueryOptions, ...params: TParams): ContractCallResult<'promise', PinkContractCallOutcome<ILooseResult<ResultType, ErrType>>>;
}
interface MapMessageInkQuery$1 {
    [message: string]: PinkContractQuery;
}
interface PinkContractOptions extends ContractOptions {
...
type PinkContractSendOptions = (PinkContractOptions & SendOptions$1 & {
    address: string | AccountId;
    signer: Signer$1;
}) | (PinkContractOptions & SendOptions$1 & {
    pair: IKeyringPair;
}) | (PinkContractOptions & SendOptions$1 & {
    provider: Provider;
});
interface PinkContractTx<TParams extends Array<any> = any[]> extends MessageMeta {
    (options: PinkContractOptions, ...params: TParams): SubmittableExtrinsic$1<'promise'>;
}
interface MapMessageTx {
...
declare class PinkContractSubmittableResult<TPinkContractPromise extends PinkContractPromise = PinkContractPromise> extends ContractSubmittableResult {
    #private;
    constructor(registry: OnChainRegistry, contract: TPinkContractPromise, nonce: string, message: AbiMessage, result: ISubmittableResult, contractEvents?: DecodedEvent[]);
    get nonce(): string;
    protected throwsOnErrorLog(chainHeight: number): Promise<void>;
    waitFinalized(predicate?: () => Promise<boolean>, options?: {
        timeout?: number;
        blocks?: number;
    }): Promise<void>;
}
interface PinkContractSend<TParams extends Array<any> = any[]> extends MessageMeta {
    (options: PinkContractSendOptions, ...params: TParams): Promise<PinkContractSubmittableResult>;
}
interface MapMessageSend {
    [message: string]: PinkContractSend;
}
type QueryProxyArgs<TParams extends Array<any> = any[]> = {
    args: TParams;
    origin?: string | AccountId | Uint8Array;
} & Partial<PinkContractQueryOptions> & unknown;
type QueryProxy<T, TParams extends Array<any> = any[], ResultType = Codec, ErrType extends Codec = Codec> = {
    [k in keyof T]: <TOverrideResultType = ResultType>(args?: QueryProxyArgs<TParams>) => ContractCallResult<'promise', PinkContractCallOutcome<ILooseResult<TOverrideResultType, ErrType>>>;
};
type TxProxyArgs<TParams extends Array<any> = any[]> = {
    args?: TParams;
    waitFinalized?: (() => Promise<boolean>) | true;
} & Partial<PinkContractSendOptions> & unknown;
...
declare class PinkContractPromise<TQueries extends PinkQueryMap = Record<string, PinkContractQuery>, TTransactions extends PinkCommandMap = Record<string, PinkContractTx>> {
    #private;
    readonly abi: Abi;
    ...
    protected _provider: AnyProvider | undefined;
    constructor(api: ApiBase<'promise'>, phatRegistry: OnChainRegistry, abi: AbiLike, address: string | AccountId, contractKey: string, provider?: AnyProvider);
    get provider(): AnyProvider | undefined;
    set provider(provider: AnyProvider);
    withProvider(provider: AnyProvider): PinkContractPromise<TQueries, TTransactions>;
    private qProxyInstance;
    get q(): { [k in keyof (TQueries & TTransactions)]: (TQueries & TTransactions)[k] extends PinkContractQuery<any[], Codec, Codec> ? <ResultType extends Codec = Codec, TParams extends any[] = any[], ErrType extends Codec = Codec>(args?: QueryProxyArgs<TParams> | undefined) => Promise<PinkContractCallOutcome<ILooseResult<ResultType, ErrType>>> : QueryProxy<(TQueries & TTransactions)[k], any[], Codec, Codec>; };
    private execProxyInstance;
    get exec(): { [k in keyof TTransactions]: TTransactions[k] extends PinkContractTx<any[]> ? <TParams extends any[] = any[]>(args?: TxProxyArgs<TParams> | undefined) => Promise<PinkContractSubmittableResult> : TxProxy<TTransactions[k], any[]>; };
    get send(): MapMessageSend;
    get registry(): Registry;
    get query(): TQueries & {
        [k in keyof TTransactions]: PinkContractQuery;
    };
    get tx(): MapMessageTx;
    private _send;
}

type AccountLike = Account | {
    address: Address;
};
interface EvmCaller {
...
interface EvmAccountMappingProviderOptions {
    SS58Prefix?: number;
}
/**
 * @class EvmAccountMappingProvider
 */
declare class EvmAccountMappingProvider implements Provider {
    #private;
    static readonly identity = "evmAccountMapping";
    constructor(api: ApiPromise, client: WalletClient, account: AccountLike, { SS58Prefix }?: EvmAccountMappingProviderOptions);
    get name(): 'evmAccountMapping';
    ready(msg?: string): Promise<void>;
    static create(api: ApiPromise, client: WalletClient, account: AccountLike, options?: {
        msg?: string;
        SS58Prefix?: number;
    }): Promise<EvmAccountMappingProvider>;
    /**
     ...
     *
     */
    send<TSubmittableResult extends SubmittableResult = SubmittableResult>(extrinsic: SubmittableExtrinsic<'promise'>, transform?: (input: ISubmittableResult) => ISubmittableResult): Promise<TSubmittableResult>;
    /**
 ...
 */
declare class KeyringPairProvider implements Provider {
    #private;
    static readonly identity = "keyring";
    constructor(api: ApiPromise, pair: KeyringPair);
    static create(api: ApiPromise, pair: KeyringPair): Promise<KeyringPairProvider>;
    static createFromSURI(api: ApiPromise, suri: string, options?: KeyringPairCreate): Promise<KeyringPairProvider>;
    get address(): string;
    get name(): 'keyring';
    /**
     * Send an extrinsic to the network.
     */
    send<TSubmittableResult extends SubmittableResult = SubmittableResult>(extrinsic: SubmittableExtrinsic<'promise', ISubmittableResult>, transform?: (input: TSubmittableResult) => TSubmittableResult): Promise<TSubmittableResult>;
    /**
     * Get a signed certificate from the account bind in the provider.
     */
    signCertificate(ttl?: number): Promise<CertificateData>;
...
type SupportedWallet = WalletConstant & {
    installed: boolean;
 ...
 */
declare class UIKeyringProvider implements Provider {
    #private;
    static readonly identity = "uiKeyring";
    constructor(api: ApiPromise, signer: Signer$1, account: InjectedAccount);
    static create(api: ApiPromise, appName: string, providerName: string, account: InjectedAccount): Promise<UIKeyringProvider>;
    get address(): string;
    get name(): 'uiKeyring';
    ...
    revokeCertificate(): void;
    adjustStake(contractId: string, amount: number): Promise<void>;
    static getSupportedWallets(): Promise<Readonly<WalletConstant[]>>;
    static getAllAccountsFromProvider(appName: string, providerName: string): Promise<InjectedAccount[]>;
}

interface InkQueryOk extends Enum {
    asInkMessageReturn: Vec<u8>;
}
interface InkQueryError extends Enum {
}
interface InkResponse extends Map {
    nonce: Vec<u8>;
...
type AnyProvider = EvmAccountMappingProvider | UIKeyringProvider | KeyringPairProvider;
interface SidevmConfig extends Struct {
    max_code_size: u32;
    max_memory_pages: u32;
    vital_capacity: u64;
    deadline: u32;
}
interface LiteralSideVmConfig {
    max_code_size: number;
    max_memory_pages: number;
    vital_capacity: number;
    deadline: number;
}
interface HookPoint extends Enum {
    readonly isOnBlockEnd: boolean;
    readonly asOnBlockEnd: null;
    readonly type: 'onBlockEnd';
}
type SystemContract = PinkContractPromise<{
    ...
    system: {
        ...
        codeExists: PinkContractQuery<[string | VecFixed<u8>, 'Ink' | 'Sidevm'], Bool>;
        codeHash: PinkContractQuery<[string | AccountId], VecFixed<u8>>;
        ...
        deploySidevmToWorkers: PinkContractQuery<[
            ...
            string | VecFixed<u8>,
            // code_hash
            string[] | Vec<VecFixed<u8>>,
            // workers
            LiteralSideVmConfig | SidevmConfig
        ]>;
    ...
    system: {
        ...
        setHook: PinkContractTx<[
            'onBlockEnd' | HookPoint,
            // hook
            string | AccountId,
            // contract
            number | `0x${string}` | u32,
            // selector
            number | bigint | u64
        ]>;
...
type LogTypeLiteral = 'Log' | 'Event' | 'MessageOutput' | 'QueryIn' | 'TooLarge';
...
interface SerMessageEventWithDecoded<Decoded extends DecodedEvent = DecodedEvent> {
    type: 'Event';
    sequence: number;
    blockNumber: number;
    contract: string;
...
interface SerMessageMessageOutputRaw {
    type: 'MessageOutput';
    sequence: number;
    blockNumber: number;
    origin: string;
...
interface SerMessageMessageOutput {
    type: 'MessageOutput';
    sequence: number;
    blockNumber: number;
    origin: string;
...
interface SerMessageQueryIn {
    type: 'QueryIn';
    sequence: number;
    user: string;
}
interface SerMessageTooLarge {
    type: 'TooLarge';
}
type SerInnerMessage = SerMessageLog | SerMessageEvent | SerMessageMessageOutputRaw | SerMessageQueryIn | SerMessageTooLarge;
type SerMessage<TEvent extends DecodedEvent = DecodedEvent> = SerMessageLog | SerMessageEventWithDecoded<TEvent> | SerMessageMessageOutput | SerMessageQueryIn | SerMessageTooLarge;
interface GetLogResponse {
    records: SerMessage[];
    next: number;
}
interface LogServerInfo {
...
declare function buildGetLogRequest(params: any[], getFrom: (x: Partial<GetLogRequest>) => number, getDefaults: () => Partial<GetLogRequest>): GetLogRequest;
type LiteralTopic = `${Capitalize<string>}::${string}`;
declare function getTopicHash(topic: LiteralTopic): string;
declare class PinkLoggerContractPromise {
    #private;
    static create(_api: ApiPromise, registry: OnChainRegistry, systemContract: SystemContract, pair?: KeyringPair): Promise<PinkLoggerContractPromise>;
    constructor(phactory: pruntime_rpc.PhactoryAPI, remotePubkey: string, pair: KeyringPair, contractId: string | AccountId, systemContractId?: string | AccountId);
    protected getSidevmQueryContext(): Promise<SidevmQueryContext>;
    /**
    ...
    getLogRaw(query?: {
        from?: number;
        count?: number;
        contract?: string | AccountId;
    }): Promise<{
        records: SerInnerMessage[];
        next: number;
    }>;
     ...
     * @deprecated
     */
    getLog(contract: AccountId | string, from?: number, count?: number): Promise<GetLogResponse>;
    /**
     * Get the logger info. It use for probe the logger status.
     */
    getInfo(): Promise<LogServerInfo>;
    /**
     * Fetching the log records, from the latest one back to the oldest one.
     */
    tail(): Promise<GetLogResponse>;
    tail(counts: number): Promise<GetLogResponse>;
    tail(request: Partial<GetLogRequest>): Promise<GetLogResponse>;
    tail(counts: number, from: number): Promise<GetLogResponse>;
    tail(counts: number, request: Omit<GetLogRequest, 'from' | 'count'>): Promise<GetLogResponse>;
    tail(counts: number, from: number, request?: Omit<GetLogRequest, 'from' | 'count'>): Promise<GetLogResponse>;
    /**
     * Fetching the log records, from the oldest one to the latest one.
     */
    head(): Promise<GetLogResponse>;
    head(counts: number): Promise<GetLogResponse>;
    head(request: Partial<GetLogRequest>): Promise<GetLogResponse>;
    head(counts: number, from: number): Promise<GetLogResponse>;
    head(counts: number, request: Omit<GetLogRequest, 'from' | 'count'>): Promise<GetLogResponse>;
    head(counts: number, from: number, request?: Omit<GetLogRequest, 'from' | 'count'>): Promise<GetLogResponse>;
    setSystemContract(contract: PinkContractPromise | string): void;
    headSystemLog(counts?: number, from?: number): Promise<GetLogResponse>;
    tailSystemLog(counts?: number, from?: number): Promise<GetLogResponse>;
}
 ...
 */
declare function createPruntimeClient(baseURL: string): pruntime_rpc.PhactoryAPI;
...
interface PartialWorkerInfo {
    clusterId?: string;
    pruntimeURL: string;
}
interface StrategicWorkerInfo {
    clusterId?: string;
...
declare class OnChainRegistry {
    ...
    getAllClusters(): Promise<(readonly [string, ClusterInfo])[]>;
    getClusterInfoById(clusterId: string): Promise<ClusterInfo | null>;
    getClusters(clusterId?: string): Promise<ClusterInfo | (readonly [string, ClusterInfo])[] | null>;
    getEndpoints(workerId?: U8aFixed | string): Promise<Option<VersionedEndpoints> | (string | Option<VersionedEndpoints>)[][]>;
    getClusterWorkers(clusterId?: string): Promise<WorkerInfo[]>;
    preparePruntimeClientOrThrows(endpoint: string): Promise<pruntime_rpc.PhactoryAPI>;
    prepareSystemOrThrows(clusterInfo: ClusterInfo): Promise<void>;
    /**
     ...
     */
    connect(worker?: WorkerInfo | PartialWorkerInfo | StrategicWorkerInfo): Promise<void>;
    ...
    get systemContract(): SystemContract | undefined;
    get alice(): KeyringPair;
    getAnonymousCert(): Promise<CertificateData>;
    resetAnonymousCert(): void;
    getClusterBalance(address: string | AccountId, cert?: CertificateData): Promise<{
        total: _polkadot_types_interfaces.Balance;
        free: _polkadot_types_interfaces.Balance;
    }>;
    ...
    get loggerContract(): PinkLoggerContractPromise | undefined;
    get remotePubkey(): string | undefined;
    get pruntimeURL(): string | undefined;
    get isEvmAccountMappingSupported(): boolean;
}

interface PinkContractInstantiateCallOutcome extends ContractCallOutcome {
    salt: string;
}
...
interface PinkInstantiateQueryOptions {
    cert: CertificateData;
    salt?: string;
    transfer?: bigint | string | number | BN;
    deposit?: bigint | string | number | BN;
}
interface PinkBlueprintOptions extends BlueprintOptions {
    deposit?: bigint | BN | string | number;
}
interface PinkBlueprintDeploy$1<ApiType extends ApiTypes> extends MessageMeta {
    (options: PinkBlueprintOptions, ...params: unknown[]): SubmittableExtrinsic$1<ApiType, PinkBlueprintSubmittableResult>;
}
interface PinkMapConstructorExec$1<ApiType extends ApiTypes> {
    [message: string]: PinkBlueprintDeploy$1<ApiType>;
}
interface SendOptions {
    cert?: CertificateData;
}
type PinkBlueprintSendOptions = (PinkBlueprintOptions & SendOptions & {
    address: string | AccountId;
    signer: Signer$1;
}) | (PinkBlueprintOptions & SendOptions & {
    pair: IKeyringPair;
}) | (PinkBlueprintOptions & SendOptions & {
    provider: Provider;
});
interface PinkBlueprintSend<TParams extends Array<any> = any[]> extends MessageMeta {
...
declare class PinkBlueprintSubmittableResult extends SubmittableResult {
    ...
    get contract(): PinkContractPromise<Record<string, PinkContractQuery<any[], _polkadot_types_types.Codec, _polkadot_types_types.Codec>>, Record<string, PinkContractTx<any[]>>>;
}
declare class PinkBlueprintPromise {
    ...
    get tx(): PinkMapConstructorExec$1<'promise'>;
    get query(): MapMessageInkQuery<'promise'>;
    get send(): MapBlueprintSend;
    private _send;
}

interface PinkCodeSendOptions {
    provider: Provider;
}
declare class InkCodeSubmittableResult extends SubmittableResult {
    ...
    waitFinalized(): Promise<void>;
    waitFinalized(timeout: number): Promise<void>;
    waitFinalized(cert: CertificateData): Promise<void>;
    waitFinalized(cert: CertificateData, timeout: number): Promise<void>;
...
declare class PinkCodePromise {
    ...
    get tx(): PinkMapConstructorExec;
    upload(): SubmittableExtrinsic<"promise", InkCodeSubmittableResult>;
    send({ provider }: PinkCodeSendOptions): Promise<InkCodeSubmittableResult>;
}
...
declare const phalaRegistryTypes: RegistryTypes;
declare const phalaTypes: TypeRegistry;
...
declare function unsafeCheckCodeHashExists(env: CheckCodeHashExistsEnv): (codeHash: string) => Promise<boolean>;
declare function unsafeGetContractCodeHash(phatRegistry: OnChainRegistry, contractId: string): Promise<string | null>;
declare function unsafeGetAbiFromPatronByCodeHash(codeHash: string): Promise<Record<string, unknown>>;
declare function unsafeGetAbiFromGitHubRepoByCodeHash(codeHash: string): Promise<Record<string, unknown>>;
declare function unsafeGetWasmFromPatronByCodeHash(codeHash: string): Promise<Uint8Array>;
declare function unsafeGetWasmFromGithubRepoByCodeHash(codeHash: string): Promise<Uint8Array>;

type SignTypedDataInput = Parameters<typeof signTypedData>[1];
declare function createEip712StructedDataSignCertificate(account: Account, encodedCert: string, ttl: number): SignTypedDataInput;
declare function createEip712StructedDataSignQuery(account: Account, encodedQuery: string): SignTypedDataInput;
 ...
 */
declare function createEip712StructedDataSubstrateCall(domain: Eip712Domain, message: SubstrateCall): Omit<SignTypedDataInput, 'account'>;

type Converters = 'SubstrateAddressConverter' | 'EvmTransparentConverter';
/**
 * Recovered pubkey and compressed pubkey.
 */
declare function recoverEvmPubkey(client: WalletClient | TestClient, account: Account, msg?: string): Promise<{
    readonly uncompressed: `0x${string}`;
    readonly compressed: `0x${string}`;
    readonly toString: () => `0x${string}`;
}>;
/**
 ...
 */
declare function evmPublicKeyToSubstrateRawAddressU8a(hex: string, converter?: Converters): Uint8Array;
declare function substrateRawAddressToEvmAddress(address: string): Hex;

declare class SignAndSendError extends Error {
    readonly isCancelled: boolean;
}
declare function signAndSend<TSubmittableResult extends SubmittableResult = SubmittableResult>(target: SubmittableExtrinsic$2<ApiTypes$1, ISubmittableResult | TSubmittableResult>, pair: AddressOrPair): Promise<TSubmittableResult>;
declare function signAndSend<TSubmittableResult extends SubmittableResult = SubmittableResult>(target: SubmittableExtrinsic$2<ApiTypes$1, ISubmittableResult | TSubmittableResult>, address: AddressOrPair, signer: Signer$1): Promise<TSubmittableResult>;
 ...
 */
declare function ackFirst(): (apiPromise: ApiPromise, clusterId: string) => Promise<Readonly<[string, string, ReturnType<typeof createPruntimeClient>]>>;

declare function periodicityChecker(url?: string): (_apiPromise: ApiPromise, clusterId: string) => Promise<Readonly<[string, string, ReturnType<typeof createPruntimeClient>]>>;

declare function fixture(endpoints: string[]): (_apiPromise: ApiPromise, _clusterId: string) => Promise<Readonly<[string, string, ReturnType<typeof createPruntimeClient>]>>;

type LiteralRpc = `ws://${string}` | `wss://${string}`;
declare function fetchMetadata(rpc: LiteralRpc): Promise<{
    [x: string]: any;
}>;
...
declare function getClient(opts: GetClientOptions): Promise<OnChainRegistry>;
...
declare function getContract<T extends PinkContractPromise>(options: GetContractOptions): Promise<T>;
type GetLoggerOptions = GetClientOptions | {
    contractId: string | AccountId;
    pruntimeURL: string;
    systemContract?: string | AccountId;
};
declare function getLogger(options: GetLoggerOptions): Promise<PinkLoggerContractPromise>;

declare const pruntimeRpc: typeof pruntime_rpc;
type PhactoryAPI = pruntime_rpc.PhactoryAPI;
...
export { type AbiLike, type AnyProvider, type CertificateData, type CheckCodeHashExistsEnv, type Converters, type CreateOptions, type Eip712Domain, EvmAccountMappingProvider, type EvmAccountMappingProviderOptions, type EvmCaller, type FrameSystemAccountInfo, type GetClientOptions, type GetContractOptions, type GetLogRequest, type GetLogResponse, type GetLoggerOptions, type HookPoint, type ILooseResult, type InjectedAccount, InkCodeSubmittableResult, type InkQueryError, type InkQueryOk, type InkResponse, type KeyringPairCreate, KeyringPairProvider, type LiteralRpc, type LiteralSideVmConfig, type LiteralTopic, type LogServerInfo, type LogTypeLiteral, type MapMessageInkQuery$1 as MapMessageInkQuery, type MapMessageSend, type MapMessageTx, OnChainRegistry, type PartialWorkerInfo, PhactoryAPI, type PinkBlueprintDeploy$1 as PinkBlueprintDeploy, type PinkBlueprintOptions, PinkBlueprintPromise, type PinkBlueprintSend, type PinkBlueprintSendOptions, PinkBlueprintSubmittableResult, PinkCodePromise, type PinkCodeSendOptions, type PinkContractCallOutcome, type PinkContractInstantiateCallOutcome, type PinkContractOptions, PinkContractPromise, type PinkContractQuery, type PinkContractQueryOptions, type PinkContractSend, type PinkContractSendOptions, type PinkContractTx, type PinkInstantiateQueryOptions, PinkLoggerContractPromise, type PinkMapConstructorExec$1 as PinkMapConstructorExec, type Provider, type SerInnerMessage, type SerMessage, type SerMessageEvent, type SerMessageEventWithDecoded, type SerMessageLog, type SerMessageMessageOutput, type SerMessageMessageOutputRaw, type SerMessageQueryIn, type SerMessageTooLarge, type SidevmConfig, SignAndSendError, type StrategicWorkerInfo, type SubstrateCall, type SupportedWallet, type SystemContract, UIKeyringProvider, UnexpectedEndpointError, type WasmLike, type WorkerInfo, abis, ackFirst, buildGetLogRequest, createEip712Domain, createEip712StructedDataSignCertificate, createEip712StructedDataSignQuery, createEip712StructedDataSubstrateCall, createPruntimeClient, createSubstrateCall, evmPublicKeyToSubstrateRawAddressU8a, fetchMetadata, fixture, generatePair, getClient, getContract, getLogger, getTopicHash, options, periodicityChecker, phalaRegistryTypes, phalaTypes, pruntimeRpc, randomHex, recoverEvmPubkey, signAndSend, signCertificate, signEip712Certificate, substrateRawAddressToEvmAddress, types, unsafeCheckCodeHashExists, unsafeGetAbiFromGitHubRepoByCodeHash, unsafeGetAbiFromPatronByCodeHash, unsafeGetContractCodeHash, unsafeGetWasmFromGithubRepoByCodeHash, unsafeGetWasmFromPatronByCodeHash };

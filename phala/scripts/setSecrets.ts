...
function logToFile(cid: string, token: string, key: string, url: string) {
  // reset to normal CID param
  const logEntry = `${new Date().toISOString()}, CID: "local", Token: [${token}], Key: [${key}], URL: [${url}]\n`;
...
try {
  const gatewayUrl = 'http://127.0.0.1:8000' //https://wapo-testnet.phala.network';
  const cid = 'local' // [UNCOMMENT FOR PROD!!!]latestDeployment.cid;
